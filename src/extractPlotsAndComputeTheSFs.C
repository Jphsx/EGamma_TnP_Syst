...
void extractPlotsAndComputeTheSFs(TString theIDname, TString dataFile, TString mcFile){
    TString tnpDir = "tpTree/";
    efficienciesDATA = new TFile(dataFile);
    efficienciesMC = new TFile(mcFile);
    TDirectory *plotDirectory = efficienciesDATA->GetDirectory(tnpDir);
    ...
    while ((keyDATA = (TKey*)listPlotsDATA())) {
        ...
        if (typeDirectory== "TDirectoryFile"){
            ...
            TDirectory *plotFinalDirectory = efficienciesDATA->GetDirectory(tnpDir+nomDirectory+"/fit_eff_plots");
	    cout << tnpDir+nomDirectory+"/fit_eff_plots" << endl;
            TIter binsEffienciesDATA(plotFinalDirectory->GetListOfKeys());
            TKey *keyEffDATA;
            /// The same key in MC will be used later
            TDirectory *plotFinalDirectoryMC = efficienciesMC->GetDirectory(tnpDir+nomDirectory+"/fit_eff_plots");
	    cout << tnpDir+nomDirectory+"/fit_eff_plots"    << endl;
	    TIter binsEffienciesMC(plotFinalDirectoryMC->GetListOfKeys());
            ...
            if (nbOfHistos==1) {
                ...
                TCanvas *theCanvas = (TCanvas*) efficienciesDATA->Get(tnpDir+nomDirectory+"/fit_eff_plots/"+nomContent);
                ...
                TCanvas *theCanvasMC = (TCanvas*) efficienciesMC->Get(tnpDir+nomDirectory+"/fit_eff_plots/"+nomContent);
                ...
                while ((keyEffDATA = (TKey*)binsEffienciesDATA())) {
                    ...
                    if ((theObj = theCanvas->FindObject(nomContent))) {
                        ...
                        for (int oneDhistos = 0 ; oneDhistos < nbYBins ; oneDhistos++ ){
                            ...
                            TCanvas *theCanvas1D = (TCanvas*) efficienciesDATA->Get(tnpDir+nomDirectory+"/fit_eff_plots/"+name1D);
                            ...
                            TCanvas *theCanvas1Dmc = (TCanvas*) efficienciesMC->Get(tnpDir+nomDirectory+"/fit_eff_plots/"+name1Dmc);
                            TIter nextObject1Dmc(theCanvas1Dmc->GetListOfPrimitives());
                            TObject *obj1Dmc = nullptr;
                            TGraphAsymmErrors *the1DAssymGraphMC = nullptr;
